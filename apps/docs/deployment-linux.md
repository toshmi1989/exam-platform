...
2. Если пользователи всё равно видят старый интерфейс: в **BotFather** → ваш бот → **Bot Settings** → **Menu Button** / **Web App URL** добавьте к URL параметр версии, например: `https://ваш-домен.ru?v=2`. Так Telegram откроет приложение по «новому» URL и подгрузит актуальный билд.
3. Пользователь может полностью закрыть Mini App (свайп вниз) и заново открыть — иногда этого достаточно.
