...
> **Это руководство по первичной установке сервера.**
> Для повседневного деплоя изменений используйте `/opt/exam/deploy.sh`.
...
# ID канала для уведомлений об оплате (можно использовать @username канала или числовой ID)
# Для получения ID: добавьте бота в канал как администратора, затем используйте @userinfobot или метод getUpdates
...
### Настройка уведомлений об оплате в Telegram канал

При успешной оплате бот автоматически отправляет уведомление в указанный канал с информацией о платеже и ссылкой на чек.

**Настройка:**

1. **Создайте канал** (если ещё нет) или используйте существующий
...
3. **Получите ID канала:**
   - Вариант 1: Используйте @userinfobot — перешлите любое сообщение из канала боту, он покажет ID
   - Вариант 2: Используйте API метод `getUpdates` после добавления бота в канал
   - ID канала обычно начинается с `-100` (например, `-1001234567890`)
   - Или используйте @username канала (например, `@my_channel`)
4. **Добавьте ID в переменные окружения:**
   ```bash
   # В apps/api/.env
   TELEGRAM_NOTIFICATION_CHANNEL_ID=-1001234567890
   # или
   TELEGRAM_NOTIFICATION_CHANNEL_ID=@my_channel
   ```
5. **Перезапустите API:**
   ```bash
   pm2 restart exam-api --update-env
   ```

**Формат уведомления:**
- Информация о пользователе (имя, username, Telegram ID)
- Тип оплаты (разовый доступ или подписка)
- Название экзамена (для разового доступа)
- Сумма оплаты
- Срок действия (для подписки)
- Инлайн кнопка со ссылкой на чек Multicard

**Примечание:** Если переменная `TELEGRAM_NOTIFICATION_CHANNEL_ID` не настроена, уведомления не отправляются, но это не влияет на обработку платежей.

### Если в Telegram Mini App видна старая версия (кэш)

Telegram и браузеры кэшируют Mini App. После деплоя:

1. **Страница чата** отдаётся с заголовком `Cache-Control: no-store`, чтобы по возможности подтягивалась свежая версия.
2. **Логотипы платежей** используют cache-busting (`?v=...`), поэтому при обновлении файлов нужно обновить константу `LOGO_VERSION` в коде страниц оплаты.
3. Если пользователи всё равно видят старый интерфейс: в **BotFather** → ваш бот → **Bot Settings** → **Menu Button** / **Web App URL** добавьте к URL параметр версии, например: `https://ваш-домен.ru?v=2`. Так Telegram откроет приложение по «новому» URL и подгрузит актуальный билд.
4. Пользователь может полностью закрыть Mini App (свайп вниз) и заново открыть — иногда этого достаточно.
